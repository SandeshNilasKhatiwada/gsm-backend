...
class RankingService {
  ...
  async calculateShopRanking(shopId) {
    ...
    totalPoints += shop.rankingPoints.reduce(
    ...
    const reviewPoints = shop.reviews.reduce((sum, review) => {
      if (review.rating === 5) return sum + RankingService.POINTS.VERIFIED_REVIEW_5_STAR;
      if (review.rating === 4) return sum + RankingService.POINTS.VERIFIED_REVIEW_4_STAR;
      if (review.rating === 3) return sum + RankingService.POINTS.VERIFIED_REVIEW_3_STAR;
      if (review.rating === 2) return sum + RankingService.POINTS.VERIFIED_REVIEW_2_STAR;
      if (review.rating === 1) return sum + RankingService.POINTS.VERIFIED_REVIEW_1_STAR;
      return sum;
    }, 0);
    ...
    const salesPoints = parseFloat(shop.totalSales) * RankingService.POINTS.SALE_MULTIPLIER;
    totalPoints += salesPoints;

    // Apply strike penalties
    const strikePenalty = shop.strikes.length * RankingService.POINTS.STRIKE_PENALTY;
    ...
    return {
      ...
      totalPoints,
      breakdown: {
        manualPoints: shop.rankingPoints.reduce((sum, rp) => sum + rp.points, 0),
        reviewPoints,
        salesPoints,
        strikePenalty,
      },
  ...
  async getShopRankingDetails(shopId) {
    ...
    const reviewStats = shop.reviews.reduce(
      (stats, review) => {
